...
echo "$($PSQL "select name from games right join teams on games.winner_id = teams.team_id where year = 2014 and round ='Eighth-Final' union select name from games right join teams on games.opponent_id = teams.team_id where year = 2014 and round ='Eighth-Final' order by name")"
